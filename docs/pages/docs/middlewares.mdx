...
those middlewares then can be added to a router class to set them globally to all procedures in the router or the class method for a specific procedure, by the `@Middlewares(){:tsx}` decorator.
The middleware decorator accepts multiple middlewares and will execute them in order, for example:
```ts
@Middlewares(LoggingMiddleware, AuthMiddleware)
```
`LoggingMiddleware` will execute first, and then `AuthMiddleware` afterward.  <br/>
When you set the `@Middlewares()` decorator for both the router and the procedure, the router will execute first and then if there aren't any duplicates, the procedure middlewares will follow after. <br/>
