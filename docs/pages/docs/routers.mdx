...
##### Middlewares
By default every route will be assigned the `publicProcedure` base, but it is possible to use custom procedures (also known as middlewares) in our router via the `@Procedure()` decorator.
<Table columns={["Decorator", "Output"]}>
  <Table.Row>
    <Table.Cell>`@Procedure(customProcedure: TRPCProcedure)`</Table.Cell>
    <Table.Cell>`customProcedure.query()`</Table.Cell>
  </Table.Row>
</Table>
You can read more about the implementation in our <Link href={"/docs/procedures"} className="underline">Procedures Guide</Link>.
