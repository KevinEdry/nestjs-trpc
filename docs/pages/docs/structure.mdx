...
<Callout> If you are looking for an installation guide for this adapter, check out our <Link href="/docs/setup" className="underline">Setup Guide</Link>.</Callout>
...
<FileTree>
  <FileTree.Folder name={"src"} open >
    ...
    <FileTree.File name={"app.procedure.ts"}/>
...
<Table>
  ...
  <Table.Row>
    <Table.Cell>`app.procedure.ts`</Table.Cell>
    <Table.Cell>Comparable to a NestJS guards or middlewares, its a procedure that can modify<br/>the request ctx.</Table.Cell>
  </Table.Row>
  <Table.Row>
